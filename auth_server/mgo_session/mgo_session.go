...
// MongoConfig stores how to connect to the MongoDB server and how long
// an ACL remains valid until new ones will be fetchemc.
type MongoConfig struct {
	DialConfig *MongoDialConfig    `yaml:"dial_info,omitempty"`
	Collection string              `yaml:"collection,omitempty"`
	CacheTTL   time.Duration       `yaml:"cache_ttl,omitempty"`
}

// Validate ensures the most common fields inside the mgo.DialInfo portion of
// a MongoConfig are set correctly as well as other fields inside the
// MongoConfig itself.
...
func New(mc *MongoConfig) (*mgo.Session, error) {
