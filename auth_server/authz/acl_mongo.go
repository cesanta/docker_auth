...
func (c *ACLMongoConfig) Validate() error {
	...
	if c.DialInfo.DialInfo.Timeout.String() == "" {
...
func (mongoAuthorizer *aclMongoAuthorizer) MaybeUpdateACLCache() error {
	...
	if len(mongoAuthorizer.cache.ACL) == 0 || duration.Seconds() > c.CacheDuration.Seconds() {
		...
		if c.DialInfo.PasswordFile != "" {
			...
			if err != nil {
				return fmt.Errorf("Failed to read password file \"%s\": %s", c.DialInfo.PasswordFile, err)
			}
			finalPassword = string(passBuf)
		}
		c.DialInfo.DialInfo.Password = finalPassword

		glog.Infof("Updating ACL from MongoDB (operation timeout %s)", c.DialInfo.DialInfo.Timeout.String())
		...
		collection := session.DB(c.DialInfo.DialInfo.Database).C(c.Collection)
