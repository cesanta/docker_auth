...
// ContinouslyUpdateACLCache checks if the ACL cache has expired and depending
...
func (ma *aclMongoAuthorizer) ContinuouslyUpdateACLCache() {
	...
	for ; true; tick = <-ma.updateTicker.C {
		...
		if err != nil {
			...
			if ma.staticAuthorizer == nil {
				glog.Infof("No stale ACL available to be re-used")
				continue
			}
		...
		// Time to acquire a write lock on the ACL cache and update it
		ma.lock.Lock()
		...
		if err != nil {
			glog.Errorf("Failed to create new static authorizer: %s", err)
			continue
		}
	}
}
