...
func (ma *aclMongoAuthorizer) updateACLCache() error {
	...
	if len(newACL) > 0 && newACL[0].Sequence == nil {
		...
		if ma.lastCacheUpdate.IsZero() {
			glog.Errorf("WARNING: Sequence not set for ACL entry: %v. As a result no ACLs have been loaded from mongodb", topACL)
		} else {
			glog.Errorf("WARNING: Sequence not set for ACL entry: %v. As a result cached acls will be used until this issue is resolved", topACL)
		}
		return nil
	}
