...
func NewRPCPlugin(a Authorizer) *RPCPlugin {
	result := &RPCPlugin{
		impl: a,
	}
...
// RPCClient implements [Authorizer] using an [rpc.Client]
// for communication.
...
func (c *RPCClient) Authorize(req *AuthorizeRequest) (resp AuthorizeResponse, err error) {
	err = c.client.Call("Plugin.Authorize", req, &resp)
...
func NewRPCServer(a Authorizer) *RPCServer {
	result := &RPCServer{
		impl: a,
	}
...
func (s *RPCServer) Authorize(req *AuthorizeRequest, resp *AuthorizeResponse) error {
	v, err := s.impl.Authorize(req)
	*resp = v
