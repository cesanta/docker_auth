...
func NewACLMongoAuthorizer(c *ACLMongoConfig) (Authorizer, error) {
	...
	if c.DialInfo.DialInfo.Password != "" {
		finalPassword = c.DialInfo.DialInfo.Password
	}
	...
	c.DialInfo.DialInfo.Password = finalPassword
...
func (ma *aclMongoAuthorizer) MaybeUpdateACLCache() error {
	...
	ma.cache.guard.RLock()
	//defer ma.cache.guard.RUnlock()

	c := ma.config

	aclAge := time.Now().Sub(ma.cache.LastUpdate)
	glog.V(2).Infof("ACL age: %s. CacheTTL: %s", aclAge, c.CacheTTL)

	// Test if cache has needs to be updated
	if aclAge < c.CacheTTL {
		glog.V(2).Infof("Using cached ACL from MongoDB (Age: %s, TTL: %s).", aclAge, ma.config.CacheTTL)
		ma.cache.guard.RUnlock()
		return nil
	}
	...
	var newCache ACL
	collection := sessionCopy.DB(c.DialInfo.DialInfo.Database).C(c.Collection)
	...
	ma.cache.ACL = newCache
	ma.cache.LastUpdate = time.Now()
	// Create a new static authorizer reusing the ACL we fetched from MongoDB
	ma.staticAuthorizer, err = NewACLAuthorizer(ma.cache.ACL)
	if err != nil {
		return err
	}
